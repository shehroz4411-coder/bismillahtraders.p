...
function initPayment() {
    ...
    // =========================
    // PAYMENT NUMBERS (APNA NUMBER DAALO)
    // =========================
    const PAYMENT_NUMBERS = {
        easypaisa: {
            name: '💳 EasyPaisa',
            number: '0317 3664422',  // ← APNA NUMBER DALO
            emoji: '📱'
        },
        jazzcash: {
            name: '💳 JazzCash',
    ...
    if (paymentOptions.length) {
        paymentOptions.forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                // Get selected method
                selectedPayment = this.dataset.method;
                const data = PAYMENT_NUMBERS[selectedPayment];

                // Update payment details
                if (data && paymentDetails) {
                    paymentDetails.classList.add('show');
                    if (methodName) {
                        methodName.textContent = data.emoji + ' ' + data.name.replace('💳 ', '');
                    }
                    if (paymentNumber) {
...
// =========================================================
// WHATSAPP CHECKOUT - WITH PAYMENT CONFIRMATION
// =========================================================
...
function checkoutWhatsApp() {
    ...
    /* PAYMENT - Get selected method */
    const selectedPayment = document.querySelector('.payment-option.active');
    const paymentMethod = selectedPayment ? selectedPayment.dataset.method : 'easypaisa';
    const paymentLabel = paymentMethod === 'easypaisa' ? 'EasyPaisa' : 'JazzCash';
    const paymentNumber = paymentMethod === 'easypaisa' ? '0317 3664422' : '0317 3664422';
    ...
    message += 'Phone: \n';
    message += 'Address: \n\n';
    message += '📸 Payment screenshot attached in next message.\n\n';
    message += 'Please confirm my order. ❤️';
    ...
    notification('✅ Order sent! Please send payment screenshot on WhatsApp.');
}